function [res]=Test_intersection_Segment_Objet (Centredegravite, Source, Triangles_ombre, Sommets_ombre)
...
for i=1:size(Triangles_ombre,1)
    if Triangles_ombre(i,4) ~= 10
        %recuperation des coordonnes des sommets de chaque triangle
        A=Sommets_ombre(Triangles_ombre(i,1),:);
        B=Sommets_ombre(Triangles_ombre(i,2),:);
        C=Sommets_ombre(Triangles_ombre(i,3),:);
